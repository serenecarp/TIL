...
public class SEOUL08_PARKSUNGJUN3 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (socket != null) {
				...
				int[] turn_1_ball = {1, 3, 5};
				int[] turn_2_ball = {2, 4, 5};
				if(order==1) {
					for(int i=0; i<turn_1_ball.length; i++) {
						if(balls[turn_1_ball[i]][0] != -1) {
							now = turn_1_ball[i];
							break;
						}
					}
				} else {
					for(int i=0; i<turn_2_ball.length; i++) {
						if(balls[turn_2_ball[i]][0] != -1) {
							now = turn_2_ball[i];
							break;
						}
					}
...
//				power = real_d;
	...
	static double getAngle(float my_x, float my_y, float target_x, float target_y) {
		...
		if (my_x > target_x && my_y > target_y) {
			...
			angle = 180 - (float) (((180.0 / Math.PI) * radian));
		}
		// 좌하단
		...
		// 좌상단
			...
			radian = Math.atan(height / width);
			angle = 360 - (float) (((180.0 / Math.PI) * radian));
		}
		return angle;
